...
export default function InvoicePage() {
  ...
  useEffect(() => {
    api.get(`/admin/orders/${id}`).then(setData).catch(() => {});
  }, [id]);
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white">
      <div className="no-print mb-4 flex justify-between items-center">
        <Link href={`/admin/orders/${id}`} className="text-brand-600 hover:underline flex items-center gap-1 text-sm">
          <ArrowLeft className="w-4 h-4" />{isAr ? 'عودة' : 'Back'}
        </Link>
      ...
      <div className="printable card p-8 border">
        ...
        <div className="flex items-start justify-between border-b pb-6 mb-6">
          <div>
            ...
            <div className="text-xs text-gray-600 space-y-0.5">
              ...
              <div className="flex items-center gap-1.5"><MapPin className="w-3 h-3" /> جمهورية مصر العربية</div>
            </div>
          </div>
          <div className="text-end">
            <div className="text-xs text-gray-500 mb-1">{isAr ? 'فاتورة' : 'INVOICE'}</div>
            <div className="text-2xl font-extrabold text-brand-900 mb-2">{order.order_number}</div>
            ...
            <div className="mt-2 text-xs">
              <span className="bg-brand-100 text-brand-700 px-2 py-0.5 rounded-full font-semibold">
                {STATUS_LABELS[locale][order.status]}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6 mb-6">
          <div>
            <div className="text-xs font-bold text-brand-700 uppercase mb-2">{isAr ? 'فاتورة إلى' : 'Bill to'}</div>
            <div className="font-bold text-brand-900">{order.customer_name}</div>
          ...
          <div>
            ...
            <div className="text-sm text-gray-700">
              {order.shipping_governorate} · {order.shipping_city}<br />
              {order.shipping_street} {order.shipping_building && `· ${order.shipping_building}`}
            </div>
          </div>
        </div>
        ...
        <div className="text-xs text-gray-500 border-t pt-4 text-center">
          ...
          <div className="mt-1">{isAr ? 'طريقة الدفع' : 'Payment'}: {order.payment_method.toUpperCase()} · {order.payment_status}</div>
        </div>
      </div>
    </div>
  );
